...
Motor output: setVelocity(v1_cmd,  v2_cmd − v1_cmd)
...
ID_WHEEL    = 2            # BEAR motor: main wheel   (lateral,       θ1 axis)
ID_ROLLER   = 1            # BEAR motor: roller       (longitudinal,  θ2 axis)
...
# ── Control gains ─────────────────────────────────────────────────────────────
# Wheel (longitudinal, pitch correction)
K11 = 85.0   # pitch angle gain   ↑ from 20 (response too slow)
K12 = 11.0   # pitchdot gain      ↑ from 1.1 (damping)
K13 =  0.0   # φ̇ gain — disabled
H1  =  0.0   # feedforward — disabled

# Roller (lateral, roll correction)
K21 = 70.0   # roll angle gain    ↑ from 14
K22 =  2.0   # rolldot gain       ↑ from 1.1
K23 =  0.0   # φ̇ gain — disabled
...
EMA_ALPHA_ANG  = 0.2    # angle  filter (~53 Hz cutoff @ 200 Hz)
EMA_ALPHA_RATE = 0.35   # rate   filter (~24 Hz cutoff @ 200 Hz)
...
# ─── Thread-safe motor velocity state ────────────────────────────────────────
class MotorState:
    def __init__(self):
        self._lock  = threading.Lock()
        self.vel_w  = 0.0   # wheel  velocity [rad/s]
        self.vel_r  = 0.0   # roller velocity [rad/s]

    def push(self, vel_w, vel_r):
        with self._lock:
            self.vel_w = vel_w
            self.vel_r = vel_r
    ...
    def get(self):
        with self._lock:
            return self.vel_w, self.vel_r


def _motor_thread(state: MotorState, robot: "Omburo", rate_hz: int = 50):
    dt = 1.0 / rate_hz
    while True:
        try:
            _, vel_w, _, vel_r = robot.readback()
            state.push(vel_w, vel_r)
        except Exception:
            pass   # keep last known value on read failure
        time.sleep(dt)
...
class Controller:
    ...
    def run(self):
        ...
        t.start()
        # motor read thread disabled: phi feedback is off (K13=K23=PI=0),
        # readback lock contention was blocking setVelocity at 200Hz
    ...
    def _step(self):
        ...
        # phi feedback disabled (K13=K23=PI=0); use zero until pure PD is verified
        phi1dot = 0.0
        phi2dot = 0.0
        ...
        self._phi1dot_filt  = a_rate * self._phi1dot_filt  + (1 - a_rate) * phi1dot
        self._phi2dot_filt  = a_rate * self._phi2dot_filt  + (1 - a_rate) * phi2dot
        ...
        # Wheel (motor 2, longitudinal): PID on pitch
        v_wheel = -(K11 * pitch + K12 * pitchdot + KI_PITCH * self._pitch_int)

        # Roller (motor 1, lateral): PID on roll, ×N_ROLLER coupling
        v_roller = (K21 * roll + K22 * rolldot + KI_ROLL * self._roll_int) * N_ROLLER

        # Saturate and apply (roller motor receives differential command)

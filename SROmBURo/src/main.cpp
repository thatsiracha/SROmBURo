...
void pingIMU();
bool getIMUData();
bool checkFletcher(uint8_t *buf, size_t len);
...
void Control(float vref1, float vref2);
...
void calc1();
void calc2();
void RemoteControl(float ch1, float ch2, float rate);
...
#define SYNC1 0x75
#define SYNC2 0x65
#define DESC_SET 0x82
#define PAY_LEN 0x20

unsigned char imu[9] = {0x75, 0x65, 0x0C, 0x03, 0x03, 0x03, 0x05, 0xF4, 0xED};
unsigned char ping[8] = {0x75, 0x65, 0x01, 0x02,0x02, 0x01, 0xE0, 0xC6};
...
float vx, vy, px, py;
// omniwheel velocity & position calculated (v*0.1144 [rpm]; p*0.088 [deg])
float phi1dot, phi2dot, phi1, phi2;
float pitchdot_last, rolldot_last, v1_last, v2_last;
// for filtering
...
// control
float vref1, vref2, psi1dot, psi2dot, theta1, theta2;
// theta1 - roll
float e1, e2, e1_int, e2_int, e1_d, e2_d, e1_prev, e2_prev;

// error gain
float K11 = 1700;
// 2300 2400 2200 2350
float K21 = 1200; // 1000 1500
...
// phidot gain (taken from servo speed?)
float K13 = 2.4; // 1.5 3 2.4
float K23 = 1.6;
// 0.8 1.6
...
float Kp1 = 0.05; // 0.05
float Ki1 = 0.01;
// 0.001 0.005 0.001 0.03 0.05
// 0.015 0.005

// Wheel strafe error
float Kp2 = 0.0035;
// 0.001 0.0008 0.0035
float Ki2 = 0.004; // 0.0015 0.0001 0.004
// 0.003 0.003
// 0.002 0.003
...
void setup() {
  ...
  Calibration();
  delay(10);
  actuate_speed(servoID1, 0.0, servoID2, 0.0);
  delay(100);
...
void pingIMU() {
  Serial1.write(ping, 8);
  Serial1.readBytesUntil(-1, data, 10);
...
bool getIMUData() {
  ...
  // Wait for IMU to send back data
  ...
  while (Serial1.available()) {
    ...
    switch (state) {
      ...
      case 4:
        data[idx++] = b;
        if (idx >= 38) {
          if (checkFletcher(data, 38)) {
            state = 0;
            return true;

          } else {
            // Try resync
            memmove(data, data + 1, 38 - 1);
            idx = 38 - 1;
            state = 0;
...
bool checkFletcher(uint8_t *buf, size_t len) {
  ...
  return (checksum_b1 == buf[len - 2] && checksum_b2 == buf[len - 1]);
}

float alpha = 1; // 0.6 0.2
void ReadOrientation() {
  
  int attempts = 50;
  while (attempts > 0) {
    if (getIMUData()) {
      pitch = floattodecimal(data[6], data[7], data[8], data[9]) - pitch_sum*1;
      // rad
      roll = floattodecimal(data[10], data[11], data[12], data[13]) - roll_sum*1;
      pitchdot = floattodecimal(data[22], data[23], data[24], data[25]); // rad
      rolldot = floattodecimal(data[26], data[27], data[28], data[29]);
      pitchdot = alpha * pitchdot + (1 - alpha) * pitchdot_last;
      pitchdot_last = pitchdot;
      rolldot = alpha * rolldot + (1 - alpha) * rolldot_last;
...
void actuate_speed(byte servoID1, int v1, byte servoID2, int v2) {
  ...
  //v1 = min(abs(v1), 1023) + 512*(1 + sign(v1));
  // CW 1024-2047 & CCW 0-1023
  ...
  PORTH |= 1 << 5; //digitalWrite(pin485,HIGH);
  //put MAX485 into transmitting mode
  Serial3.write(motorspeed, 14);
  Serial3.flush(); // critical!!!!!!!!!
  PORTH &= 0 << 5; //digitalWrite(pin485,LOW);
  //put MAX485 back into receiving mode
}
...
void Control(float vref1, float vref2) {
  ...
  e1_int += e1 * dt;
  e2_int += e2 * dt;
  theta1 = - 0.13 * 0 + roll - Kp1 * e1 - Ki1 * e1_int;
  ...
  // Serial.print(K21 *Kp2 * e2 * 4);
  Serial.print(K21 * theta2);
  Serial.print(" ");
  Serial.print(K22 * Ki2 * e2_int * 4);
  Serial.println();
}
...
void VelocityTrack() {
  if (millis() > 8000)
  {
    vref1 = 0.04 * (millis() - 8000) / 1000;
    //vref1 = 0.2;
    if (millis() > 7000 + 5000 + 1000)
    {
      vref1 = 0.2;
      if (millis() > 7000 + 5000 + 11000)
      {
        vref1 = 0.2 - 0.04 * (millis() - 23000) / 1000;
        //vref1 = 0.2;
        if (millis() > 7000 + 5000 + 10000 + 5000 + 1000)
        {
          vref1 = 0;
        }
      }
...
void RemoteControl(float ch1, float ch2, float rate) {
  if (ch1 - 1500 > 20)
  {
    vref1 = (ch1 - 1520) / 2500 * rate;
  }
  else if (ch1 - 1500 < -20)
  {
    vref1 = (ch1 - 1480) / 2500 * rate;
  }
  else
  {
    vref1 = 0;
  ...
  if (ch2 - 1500 > 20)
  {
    vref2 = (ch2 - 1520) / 2500 * rate / 2;
  }
  else if (ch2 - 1500 < -20)
  {
    vref2 = (ch2 - 1480) / 2500 * rate / 2;
  }
  else
  {
    vref2 = 0;
